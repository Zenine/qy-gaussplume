...
public class SimulationControllerTests : IDisposable
{
    ...
    private async Task<EmissionSourceDto> CreateAreaSource(double lat, double lon, double length, double width) =>
        await (await _client.PostJsonAsync("/api/sources", new EmissionSourceCreateDto
        {
    ...
    private static double AxisSpan(IReadOnlyCollection<double> values) => values.Max() - values.Min();
    ...
    public async Task 单源远受体_浓度场网格中心保持污染源中心()
    {
        ...
        await CreateReceptor(lat: 40.500, lon: 117.300);
        ...
        var result = await resp.ReadJsonAsync<SimulationResultDto>();
        AxisCenter(result.GridLat).Should().BeApproximately(39.900, 1e-8);
        AxisCenter(result.GridLon).Should().BeApproximately(116.400, 1e-8);
    }
    ...
    public async Task 单源调大模拟范围_网格扩大但中心不变()
    {
        ...
        var bigResp = await _client.PostJsonAsync("/api/simulation/run", new SimulationRequestDto
        {
            MeteorologyId = met.Id,
            GridResolution = 100,
            DomainSize = 20_000,
        ...
        var big = await bigResp.ReadJsonAsync<SimulationResultDto>();

        AxisCenter(small.GridLat).Should().BeApproximately(39.900, 1e-8);
        AxisCenter(big.GridLat).Should().BeApproximately(39.900, 1e-8);
        AxisCenter(small.GridLon).Should().BeApproximately(116.400, 1e-8);
        AxisCenter(big.GridLon).Should().BeApproximately(116.400, 1e-8);
        AxisSpan(big.GridLat).Should().BeGreaterThan(AxisSpan(small.GridLat));
        AxisSpan(big.GridLon).Should().BeGreaterThan(AxisSpan(small.GridLon));
    }
    ...
    public async Task 多源远受体_浓度场网格中心使用参与源外包框中心()
    {
        var met = await CreateMet();
        await CreatePointSource(lat: 39.900, lon: 116.400);
        await CreatePointSource(lat: 39.940, lon: 116.480);
        await CreateReceptor(lat: 40.500, lon: 117.300);
    ...
    {
        var met = await CreateMet();
        await CreateAreaSource(lat: 39.900, lon: 116.400, length: 2_000, width: 1_000);
        await CreateReceptor(lat: 40.500, lon: 117.300);
        ...
        var result = await resp.ReadJsonAsync<SimulationResultDto>();
        var latSpanMeters = AxisSpan(result.GridLat) * 111_000;
        var lonSpanMeters = AxisSpan(result.GridLon) * 111_000
            * Math.Cos(AxisCenter(result.GridLat) * Math.PI / 180.0);

        AxisCenter(result.GridLat).Should().BeApproximately(39.900, 1e-8);
        AxisCenter(result.GridLon).Should().BeApproximately(116.400, 1e-8);
        latSpanMeters.Should().BeGreaterThan(1_900);
        lonSpanMeters.Should().BeGreaterThan(1_900);
    }
    ...
    {
        var met = await CreateMet();
        await CreateLineSource(startLat: 39.900, startLon: 116.400, endLat: 39.940, endLon: 116.480);
        await CreateReceptor(lat: 40.500, lon: 117.300);
    ...
    public async Task 网格范围_以污染源为中心且不小于用户模拟范围()
    {
        ...
        AxisCenter(result.GridLat).Should().BeApproximately(39.900, 1e-8);
        AxisCenter(result.GridLon).Should().BeApproximately(116.400, 1e-8);
        latRangeMeters.Should().BeInRange(49_000, 51_000);
        lonRangeMeters.Should().BeInRange(49_000, 51_000);
    }
}
