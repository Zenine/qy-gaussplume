...
| 验证结果 | Vitest 123 个用例，`npm run build` 通过 |
...
- **污染源中心化范围截断**：主控台只使用启用的排放源和受体点；后端单风向、多风向浓度场都以参与排放源外包框中心生成模拟范围，避免远受体点拉偏热力图。
...
这张图对应当前 GNN 主控台：地图为主画布，顶部工具条负责固定区域切换、图层、气象场、本次计算污染物和模拟动作，左下角控制模拟范围、网格分辨率与模拟高度，右侧悬浮卡片承载区域绘制、气象控制、数据统计和运行后结果分析。结果卡与贡献排名里的污染物选择只切换已有结果展示；贡献排名按“空气站点 → 污染物指标 → 污染源”分组展示，不改变下一次计算请求。
...
| `npm test` | Vitest 一次跑完全部 123 个用例 |
...
- **`/api/{regions|sources|receptors|meteorology|simulation|map|config}/...`**：所有调用通过 `src/api/*.ts` 封装；排放源、受体点、气象场列表/创建/导入会带当前固定区域 `regionKey`
...
- 主控台固定 4 个项目区域：南湖区、秀洲区、嘉善县、桐乡市；切换区域后排放源、受体点、气象场和本地模拟结果互相隔离。
