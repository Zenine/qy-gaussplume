...
export const meteorologyApi = {
  list: (skip = 0, limit = 100, regionKey?: string) =>
    http.get<Meteorology[]>('/api/meteorology', { params: { skip, limit, regionKey } }).then((r) => r.data),

  get: (id: number) => http.get<Meteorology>(`/api/meteorology/${id}`).then((r) => r.data),

  create: (payload: MeteorologyCreate, regionKey?: string) =>
    http.post<Meteorology>('/api/meteorology', payload, { params: { regionKey } }).then((r) => r.data),

  createBatch: (payload: MeteorologyCreate[], regionKey?: string) =>
    http.post<Meteorology[]>('/api/meteorology/batch', payload, { params: { regionKey } }).then((r) => r.data),
