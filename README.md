...
| 验证结果 | 后端 148 个用例、前端 108 个用例，`scripts/verify.sh` 通过 |
...
- **同步回归测试**：新增主控台风向指针、排放源/气象场批量删除、多污染因子、多风向空 ID、等效面源贡献、非等权聚合和所有风向失败测试。
...
- **贡献分析与公式说明**：按受体点和污染物查看污染源贡献排名与百分比，首页可快速查看当前受体点贡献摘要，并可打开公式抽屉确认不同污染因子的计算参数。
...
当前测试规模：后端 148 个用例，前端 108 个用例，合计 256 个自动化测试。
