...
- 主控台：地图点位、启用数据过滤、框选、单风向/多风向模拟、XLSX 风频导入对话框（模板下载、文件选择、72 行数据预览与权重和确认）、逐方位平均风速/权重聚合、右侧气象控制圆盘、风速/来风方向临时调整、浓度场叠加、扩散浓度色阶控制、空气站点污染源贡献排名、贡献抽屉入口。
- 主控台顶部工具条采用上下两行：基础选择与风频导入位于第一行，模拟参数与运行操作位于第二行；窄视口下行内自动换行，不再要求横向拖动。顶部计算污染物只影响下一次模拟，已有结果仍由结果卡片的显示污染物控制。
- 排放源管理：列表、新增/编辑、删除、批量删除、全部启用/全部停用、类型筛选确认、按类型模板下载/批量导入、源类型专属字段和污染物子表。
- 等效面源污染物只显示一个实测浓度数值框；线源地图显示为连续双层圆角线带，线源浓度图仅做轻微视觉柔化，原始浓度和受体贡献仍由后端 FLSI 有限长线源积分法计算。
- 排放源和受体点表单从后端加载完整标记图标目录，地图上的点源与受体点按所选图标和颜色显示；接口不可用时使用同内容的本地回退目录。
- 新增 SO2（二氧化硫）业务污染物，支持排放源录入、Excel 导入、单/多风向计算、结果切换与公式参数展示。
- 受体点管理：列表、新增/编辑、删除、批量删除、下载模板、批量导入、导出已选、全部启用/全部停用。
- 气象场管理：列表、新增/编辑、删除、批量删除、全部启用，以及边界层高度、温度、湿度、云量、降水等完整字段。

后续如需做到与 Vue3 版本完全像素级一致，建议继续补齐：公式抽屉精细排版和更多组件级自动化测试。
